...
- Blender 5.2.0 LTS
- Long-lens perspective camera: 10° vertical FOV, position `(-18.93, 2.31, 0.9)`, target `(0.0, 0.65, 0.9)`
- Camera looks from the room entrance toward the bar/backbar wall, square to that wall, at approximately 5° downward; the long lens suppresses—but deliberately does not remove—perspective convergence, and the true foreground wall is hidden as a cutaway
- Sprite: 0.025 meters per source pixel, nearest filtering, alpha discard, fixed-Y billboard; its center is raised to `Y = 0.4` so the opaque feet meet the floor and its visible height reads at roughly 1.6 times the adjacent counter height
- Source units: metric at 1.0; preserve 1 Blender unit = 1 Godot meter
- Lighting: neutral ambient for material separation, neutral-warm directional key, restrained local bar light, SSAO, and a small contact shadow under the sprite
...
- `mirror` imports as an opaque black panel. The spike replaces it by name with a lighter Godot metallic/emissive material so it remains legible; production should use a measured reflection treatment.
...
- Keep Forward+, the wall-flush long-lens perspective camera with the bar at the back of the room, foreground-wall cutaway, fixed-Y sprite billboard, nearest filtering, meter scale, copied-texture layout, and named Godot material-override strategy.
