#!/usr/bin/env bash
# macOS/Linux equivalent of perception_danger/run_slice.ps1
# Runs lean tests, captures the five perception stages, writes a validation report,
# and fails if the report's "passed" flag is not true.
# Usage: tools/perception_danger/run_slice.sh [--godot-bin /path/to/Godot]
set -euo pipefail
...
ARTIFACT_DIR="$PROJECT_ROOT/artifacts/perception_danger"
mkdir -p "$ARTIFACT_DIR"

"$PROJECT_ROOT/tools/test_headless.sh" --godot-bin "$GODOT_BIN"

isolate_profile "$PROJECT_ROOT/.godot/perception_danger_profile"
...
for STAGE in line_of_sight room_hearing unattended_body companion debug_trace; do
  "$GODOT_BIN" --path "$PROJECT_ROOT" --fixed-fps=60 --disable-vsync \
    ...
    "--stage=$STAGE" "--capture=res://artifacts/perception_danger/${STAGE}.png"
done

"$GODOT_BIN" --headless --path "$PROJECT_ROOT" "$SCENE" -- \
  "--stage=debug_trace" "--report=res://artifacts/perception_danger/validation.json"
...
if [[ "$passed" != "true" ]]; then
  echo "Perception & Danger validation checks reported a failure." >&2
  exit 1
fi

echo "Perception & Danger evidence written to $ARTIFACT_DIR"
