...
$Frames_ = @(
    ...
    @{ Name = "inspected";       Extra = @("--inspect-patron=opening_patron", "--hud-preview=quiet") },
    ...
    @{ Name = "controls_card"; Frames = 60; Extra = @("--hud-preview=controls") },
...
foreach ($Frame in $Frames_) {
    ...
    & $GodotBin --path $ProjectRoot --fixed-fps=60 --disable-vsync --rendering-method gl_compatibility --resolution 1280x720 $Scene -- `
        "--stage=full_cast" "--capture-frames=$FrameCount" "--identify-patron=opening_patron" `
...
foreach ($Responsive in @(
    ...
    @{ Name = "controls_card_1024"; Resolution = "1024x576"; Preview = "controls" },
    @{ Name = "outcome_success_1024"; Resolution = "1024x576"; Preview = "outcome_victory" },
    @{ Name = "outcome_failed_1024"; Resolution = "1024x576"; Preview = "outcome_failed" },
    @{ Name = "outcome_exposed_1024"; Resolution = "1024x576"; Preview = "outcome_exposed" }
)) {
    ...
    & $GodotBin --path $ProjectRoot --fixed-fps=60 --disable-vsync --rendering-method gl_compatibility `
        ...
        "--identify-patron=opening_patron" "--inspect-patron=opening_patron" @($Preview) `
        "--capture=$CapturePath"
