...
func test_seeded_choice_and_fifo_cleanup_are_repeatable() -> void:
	...
	for scenario in [first, replay]:
		...
		scenario.add_patron(1001, 100.0)
		scenario.check_bathroom_choice(1001)
	...
	for patron_id in [1001, 1002, 1003]:
		first.add_patron(patron_id, 100.0)
		assert_true(first.force_bathroom_intent(patron_id))
	var queued: Dictionary = first.snapshot()
	assert_eq(queued["occupant_id"], 1001)
	assert_eq(queued["queue"], [1002, 1003])

	assert_true(first.cancel_actor(1001))
	assert_eq(first.snapshot()["occupant_id"], 1002)
	assert_eq(first.snapshot()["queue"], [1003])
	assert_true(first.cancel_actor(1002))
	assert_true(first.cancel_actor(1003))
...
func test_missing_companion_reaches_investigation_intercept_and_defeat_once() -> void:
	...
	scenario.advance(2.05)
	assert_true(scenario.begin_intercept(COMPANION, 1))
	assert_false(scenario.begin_intercept(COMPANION, 2))
	scenario.advance(5.05)
