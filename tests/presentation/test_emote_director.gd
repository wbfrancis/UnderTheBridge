...
func test_every_persistent_state_appears_from_its_source_and_ends_with_it() -> void:
	...
	for state: StringName in states:
		director.reset()
		director.update(_view([_row(1001, state)]), 0.1, false)
		assert_eq(_shown(director).get(1001, &""), state,
			"%s must show its own bubble." % state)
		director.update(_view([_row(1001, &"none")]), 0.1, false)
		assert_false(_shown(director).has(1001),
			"%s must disappear when its source state ends." % state)


func test_an_absent_actor_shows_nothing() -> void:
	var director = _director()
	var row := _row(1001, &"ordering")
...
func test_escape_preempts_everything_and_investigation_preempts_ordinary_states() -> void:
	...
	director.update(_view([_row(1001, &"ordering", calm)]), 0.1, false)
	# A Mood drop queues a transient over the ordinary Ordering state.
	director.update(_view([
		_row(1001, &"ordering", {"satisfaction": "Unhappy", "danger": "Calm", "rapport": "Stranger"})
	]), 0.1, false)
	assert_eq(_shown(director)[1001], &"mood_down",
		"A transient reads over an ordinary persistent state.")

	director.update(_view([_row(1001, &"escaping")]), 0.1, false)
	assert_eq(_shown(director)[1001], &"escaping", "Escape preempts every other bubble.")

	var second = _director()
	second.update(_view([_row(1002, &"bathroom")]), 0.1, false)
	second.update(_view([_row(1002, &"investigating")]), 0.1, false)
	assert_eq(_shown(second)[1002], &"investigating",
...
func test_a_critical_state_suppresses_transients_and_none_resume_afterwards() -> void:
	var director = _director()
	var calm := {"satisfaction": "Content", "danger": "Calm", "rapport": "Stranger"}
	director.update(_view([_row(1001, &"none", calm)]), 0.1, false)
	# The Mood drop arrives in the same update that turns the Patron unconscious.
	director.update(_view([
		_row(1001, &"unconscious", {"satisfaction": "Miserable", "danger": "Calm", "rapport": "Stranger"})
	]), 0.1, false)
	assert_eq(_shown(director)[1001], &"unconscious")

	director.update(_view([_row(1001, &"none", {"satisfaction": "Miserable", "danger": "Calm", "rapport": "Stranger"})]), 0.1, false)
	assert_false(_shown(director).has(1001),
...
func test_one_actor_never_shows_two_bubbles_or_holds_more_than_two_transients() -> void:
	...
	var pending: Array = director.snapshot()["pending"][1001]
...
func test_repeated_events_of_one_kind_are_deduplicated() -> void:
	var director = _director()
	director.update(_view([_row(1001, &"none")]), 0.1, false)
	director.update(_view([_row(1001, &"none", {}, [&"service_smile", &"service_smile"])]), 0.0, false)
	director.update(_view([_row(1001, &"none", {}, [&"service_smile"])]), 0.0, false)

	assert_eq(_shown(director)[1001], &"service_smile")
	assert_true(director.snapshot()["pending"][1001].is_empty(),
...
func test_transients_use_real_time_so_four_times_play_does_not_shorten_them() -> void:
	var director = _director()
	director.update(_view([_row(1001, &"none")]), 0.1, false)
	director.update(_view([_row(1001, &"none", {}, [&"service_frown"])]), 0.0, false)
	assert_eq(_shown(director)[1001], &"service_frown")

	# Two real seconds, delivered as frames at any simulation speed.
	director.update(_view([_row(1001, &"none")]), 1.7, false)
	assert_eq(_shown(director)[1001], &"service_frown", "The transient still has time left.")
	director.update(_view([_row(1001, &"none")]), 0.4, false)
	assert_false(_shown(director).has(1001), "The transient ends on its real duration.")
...
func test_pause_freezes_transient_timers() -> void:
	...
	for _frame in range(20):
		director.update(_view([_row(1001, &"none")]), 1.0, true)
	assert_eq(_shown(director)[1001], &"service_frown",
...
func test_reset_clears_every_bubble_for_a_new_night() -> void:
	var director = _director()
	director.update(_view([_row(1001, &"escaping"), _row(1002, &"ordering")]), 0.1, false)
...
func test_bubble_order_is_deterministic_by_priority_then_actor_id() -> void:
	...
	assert_eq(order, [1002, 1001, 1003])
...
func test_every_bubble_carries_an_icon_a_silhouette_and_a_label() -> void:
	...
	for emote: StringName in load(EMOTE_DIRECTOR_PATH).CATALOG:
		director.reset()
		director.update(_view([_row(1001, &"none")]), 0.1, false)
		director.update(_view([_row(1001, emote, {}, [emote])]), 0.0, false)
...
func test_the_emote_view_carries_no_hidden_simulation_value() -> void:
	...
	session.report_patron_stimulus(5, &"drink_dosed_seen")
	session.advance(2.2)
...
func test_unknown_patrons_show_bubbles_without_leaking_profile_fields() -> void:
	var session = _session_with_first_group()
	var row: Dictionary = session.snapshot()["emote_view"][4]
	...
	assert_eq(_shown(director)[4], &"ordering")


func test_both_collapse_causes_show_the_same_public_unconscious_bubble() -> void:
	var overdrink = _session_with_first_group()
	overdrink.debug_set_patron_drink_state(4, 3, 1, 0, 3)
	overdrink.debug_force_finish_drink(4)
	overdrink.advance(0.2)

	var drugged = _session_with_first_group()
	drugged.prepare_drugged_drink(4, 1)
	drugged.advance(8.1)
	drugged.serve_patron_order(4, 1)
	drugged.advance(21.0)

	var overdrink_row: Dictionary = overdrink.snapshot()["emote_view"][4]
	var drugged_row: Dictionary = drugged.snapshot()["emote_view"][4]
...
func test_ordinary_move_needs_no_cultist_bubble() -> void:
	...
	assert_eq(session.emote_view(moving)[1]["state"], &"none",
	...
	assert_eq(session.emote_view(working)[1]["state"], &"none",
...
func test_a_finished_command_produces_no_emote_event() -> void:
	...
	var row: Dictionary = session.emote_view(finished)[2]
	...
	assert_false(_shown(director).has(2))
