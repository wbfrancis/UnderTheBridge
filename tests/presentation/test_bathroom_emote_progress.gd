...
func _forced_bathroom_session():
	...
	session.start(707)
	session.advance(1.1)
	session.debug_force_bathroom(&"patron_june")  # headless: arrives instantly
	return session


func _bubble(session, director) -> Dictionary:
	director.update({&"patron_june": session.patron_emote_row(&"patron_june")}, 0.0, false)
	for entry: Dictionary in director.bubbles():
		if entry["actor_id"] == &"patron_june":
			return entry
	return {}
...
func test_a_locked_policy_row_stays_a_generic_bathroom_emote_with_no_fill() -> void:
	...
	director.update({&"patron_june": {
		"id": &"patron_june", "kind": &"patron", "present": true, "state": &"bathroom",
...
func test_prototype_progress_leaks_no_forbidden_debug_fields() -> void:
	...
	var row := session.patron_emote_row(&"patron_june")
...
func test_the_ratio_tracks_simulated_time_regardless_of_chunking() -> void:
	...
	for _step in range(25):
		chunked.advance(0.1)
	assert_almost_eq(
		float(one_shot.patron_emote_row(&"patron_june")["progress"]["ratio"]),
		float(chunked.patron_emote_row(&"patron_june")["progress"]["ratio"]),
...
func test_pause_freezes_the_fill_because_the_simulation_is_paused() -> void:
	...
	var before: float = session.patron_emote_row(&"patron_june")["progress"]["ratio"]
	# A Plain Pause simply stops advancing the session; the ratio cannot drift.
	var after: float = session.patron_emote_row(&"patron_june")["progress"]["ratio"]
...
func test_overlay_renders_one_bottom_to_top_fill_for_the_active_phase() -> void:
	...
	var bubble := {
		"actor_id": &"patron_june", "emote": &"bathroom", "category": &"persistent",
		"priority": 60, "icon": "T", "shape": &"square", "color": "7fc7c4",
		"label": "Toilet", "progress_phase": &"toilet", "progress_ratio": 0.6,
	}
	overlay.refresh([bubble], {&"patron_june": Vector3.ZERO})
	assert_almost_eq(overlay.fill_ratio(&"patron_june"), 0.6, 0.001,
		"The overlay fills to the phase ratio.")
	var rect: Rect2 = overlay.placements()[&"patron_june"]
