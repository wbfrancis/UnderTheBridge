...
func test_normal_command_replaces_and_shift_command_appends() -> void:
	...
	var first: Dictionary = commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), false)
	var queued: Dictionary = commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-3.0, 4.0), true)
	var replacement: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"move", _floor_target(2.0, 4.0), false
	)
...
func test_a_large_queue_is_never_rejected_for_its_length() -> void:
	...
	for index in range(40):
		var appended: Dictionary = commands.issue(
			ActorIds.CULTIST_IDS[0], &"move", _floor_target(float(index), 4.0), true
		)
	...
	assert_eq(int(_queue(commands, ActorIds.CULTIST_IDS[0])["action_count"]), 41)
...
func test_mixed_move_and_context_commands_keep_the_order_they_were_queued() -> void:
	...
	commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), false)
	commands.issue(ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), true, adjacent)
	commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-2.0, 4.0), true)
	commands.issue(ActorIds.CULTIST_IDS[0], &"offer_cigarette", _patron_target(ScenarioActors.opening_companion()), true, adjacent)
	commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(4.0, 4.0), true)
...
func test_pending_actions_can_be_removed_without_touching_the_active_action() -> void:
	...
	var active: Dictionary = commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), false)
	var removable: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), true
	)

	assert_true(commands.remove_pending(ActorIds.CULTIST_IDS[0], int(removable["action_id"])))
	var queue := _queue(commands, ActorIds.CULTIST_IDS[0])
...
func test_precommit_cancellation_releases_the_reservation_and_changes_nothing() -> void:
	...
	commands.issue(ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), false)
	assert_eq(
		commands.snapshot()["reserved_slots"].get(ActorIds.CULTIST_IDS[0], &""),
		StringName("approach_%d" % 4),
		"An approach is reserved before movement starts."
	)

	commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), false)
	assert_false(commands.snapshot()["reserved_slots"].has(ActorIds.CULTIST_IDS[0]),
...
func test_active_cancellation_before_commitment_releases_and_starts_the_next_action() -> void:
	...
	var queued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), true
	)
	assert_eq(
		commands.snapshot()["reserved_slots"].get(ActorIds.CULTIST_IDS[0], &""),
		StringName("approach_%d" % 4),
	...
	var cancelled: Dictionary = commands.request_cancel_active(ActorIds.CULTIST_IDS[0])
...
func test_cancellation_cannot_undo_an_action_past_its_commitment_point() -> void:
	...
	var issued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), false, {"is_adjacent": true}
	)
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(issued["action_id"]))
	assert_false(session.snapshot()["conversations"].is_empty(),
		"The Talk committed, so the conversation is running.")

	var refused: Dictionary = commands.request_cancel_active(ActorIds.CULTIST_IDS[0])
...
func test_cancelling_the_last_action_leaves_an_idle_queue_and_no_reservation() -> void:
	...
	commands.issue(ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), false)
	assert_true(bool(commands.request_cancel_active(ActorIds.CULTIST_IDS[0])["cancelled"]))
...
func test_the_normal_snapshot_marks_each_action_cancellable_without_hidden_state() -> void:
	...
	commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), true)
	var queue := _queue(commands, ActorIds.CULTIST_IDS[0])
...
func test_the_effect_fires_once_at_commitment_and_a_later_command_cannot_reverse_it() -> void:
	...
	var issued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"make_wine", _object_target(&"bar_work_position"), false
	)
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(issued["action_id"]))
	...
	assert_true(commands.snapshot()["cultists"][ActorIds.CULTIST_IDS[0]]["active"]["command"] == &"move")
...
func test_a_stale_target_fails_visibly_and_advances_the_queue() -> void:
	...
	session.debug_force_finish_drink(ScenarioActors.opening_patron())
	session.advance(0.2)
	commands.refresh(ActorIds.CULTIST_IDS[0])
...
func test_navigation_failure_releases_the_reservation_and_reports_a_reason() -> void:
	...
	commands.notify_failed(ActorIds.CULTIST_IDS[0], int(issued["action_id"]), &"path_stuck")
...
func test_three_cultists_can_work_at_the_bar_at_the_same_time() -> void:
	...
	var second: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[1], &"make_beer", _object_target(&"bar_work_position"), false
	)
	var third: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[2], &"make_liquor", _object_target(&"bar_work_position"), false
	)

	assert_true(bool(first["accepted"]))
	assert_true(bool(second["accepted"]))
	assert_true(bool(third["accepted"]))
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(first["action_id"]))
	commands.notify_reached(ActorIds.CULTIST_IDS[1], int(second["action_id"]))
	commands.notify_reached(ActorIds.CULTIST_IDS[2], int(third["action_id"]))
	commands.advance(5.0)
...
func test_admission_survives_command_ticks_and_releases_after_group_entry() -> void:
	...
	var issued: Dictionary = commands.issue(ActorIds.CULTIST_IDS[0], &"admit_group", _object_target(&"front_entrance"), false)
	assert_true(issued["accepted"])
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(issued["action_id"]))
	commands.advance(0.1)
	assert_false(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty(), "The active admission must not reject itself as Busy.")
	for step in range(50):
		session.advance(0.1)
		commands.advance(0.1)
	assert_eq(session.snapshot()["debug_patron_views"][ScenarioActors.opening_patron()]["activity"], &"awaiting_drink")
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty(), "The committed door hold ends when the group enters.")
...
func test_nonadjacent_talk_creates_a_visible_move_then_talk_chain() -> void:
	...
	var issued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), false
	)
	assert_true(bool(issued["accepted"]))
	assert_eq(issued["generated_action_ids"].size(), 1,
		"A nonadjacent Talk gains one Generated Move prerequisite.")
	assert_eq(int(issued["action_id"]), int(_queue(commands, ActorIds.CULTIST_IDS[0])["pending"][0]["id"]),
...
func test_shift_appends_a_full_chain_after_unrelated_work() -> void:
	...
	commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), false)
	commands.issue(ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), true)
	var queue := _queue(commands, ActorIds.CULTIST_IDS[0])
...
func test_a_patron_that_moves_before_activation_inserts_a_generated_move() -> void:
	...
	var outcome: Dictionary = commands.resolve_proximity(
		ActorIds.CULTIST_IDS[0], int(issued["action_id"]), false
	)
...
func test_generated_move_and_dependent_action_share_one_reservation() -> void:
	...
	assert_eq(commands.snapshot()["reserved_slots"].get(ActorIds.CULTIST_IDS[0], &""),
		StringName("approach_%d" % 4), "The Generated Move reserves the Approach Position.")
	var move_id := int(commands.active_request(ActorIds.CULTIST_IDS[0])["action_id"])
	commands.notify_reached(ActorIds.CULTIST_IDS[0], move_id)
	assert_eq(commands.snapshot()["reserved_slots"].get(ActorIds.CULTIST_IDS[0], &""),
		StringName("approach_%d" % 4),
		"The reservation stays through the chain with no release gap.")
	var talk_id := int(issued["action_id"])
	assert_true(bool(commands.resolve_proximity(ActorIds.CULTIST_IDS[0], talk_id, true)["committed"]),
...
func test_cancelling_a_chain_tile_removes_all_links_and_releases_reservation() -> void:
	...
	assert_true(commands.remove_pending(ActorIds.CULTIST_IDS[0], int(issued["action_id"])))
	var queue := _queue(commands, ActorIds.CULTIST_IDS[0])
...
func test_navigation_failure_cancels_dependent_links_and_starts_the_next_action() -> void:
	...
	var move_id := int(commands.active_request(ActorIds.CULTIST_IDS[0])["action_id"])
	commands.notify_failed(ActorIds.CULTIST_IDS[0], move_id, &"path_stuck")
...
func test_no_queue_full_reason_appears_in_normal_behavior() -> void:
	...
	for index in range(12):
		commands.issue(ActorIds.CULTIST_IDS[0], &"move", _floor_target(float(index), 4.0), true)
...
func test_talk_commits_and_is_unavailable_for_a_missing_patron() -> void:
	...
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()))["committed"]))
	assert_eq(session.snapshot()["conversations"].get(ActorIds.CULTIST_IDS[0], &""), ScenarioActors.opening_patron())
	assert_eq(
		_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["command"],
		&"talk",
		"Talk remains the active Action while the conversation continues."
	)

	assert_true(_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.group_member(&"arrival_group_pair_02", 1, 2)), &"talk").is_empty(),
...
func test_replacing_talk_ends_it_but_shift_appended_work_waits() -> void:
	...
	assert_true(bool(_run(
		commands, ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron())
	)["committed"]))
	var appended: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0), true
	)
	var waiting := _queue(commands, ActorIds.CULTIST_IDS[0])
	...
	assert_true(session.snapshot()["conversations"].is_empty())
	var replaced := _queue(commands, ActorIds.CULTIST_IDS[0])
...
func test_service_uses_a_reserved_drink_pickup_move_and_handoff_chain() -> void:
	...
	assert_true(
		_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.opening_patron()), &"serve_order").is_empty(),
		"Patrons no longer expose the direct Serve Order command."
	)
	var drink_type: StringName = session.patron_view(ScenarioActors.opening_patron(), ActorIds.CULTIST_IDS[0])["ordered_drink"]
	var make_command := StringName("make_%s" % drink_type)
	var make: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], make_command, _object_target(&"bar_work_position"), false
	)
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(make["action_id"]))
	commands.advance(5.0)
	var drink_id: StringName = session.snapshot()["prepared_drinks"]["drinks"][0]["id"]
	var service: Dictionary = commands.issue_drink_service(
		ActorIds.CULTIST_IDS[0], drink_id, _patron_target(ScenarioActors.opening_patron()), false
	)
	assert_true(bool(service["accepted"]))
	assert_eq(_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["command"], &"pick_up_drink")
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(service["generated_action_ids"][0]))
	commands.advance(1.0)
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(service["generated_action_ids"][1]))
	commands.resolve_proximity(ActorIds.CULTIST_IDS[0], int(service["action_id"]), true)
	commands.advance(1.0)
	assert_eq(int(session.snapshot()["orders"]["served_count"]), 1)
	assert_true(session.prepared_drink(drink_id).is_empty())
	assert_false(session.is_cultist_busy(ActorIds.CULTIST_IDS[0]),
...
func test_drink_menu_exposes_service_drugging_and_disposal() -> void:
	...
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(preparation["action_id"]))
	commands.advance(5.0)
	var drink_id: StringName = session.snapshot()["prepared_drinks"]["drinks"][0]["id"]
	var options: Array[Dictionary] = commands.resolve_drink_options(drink_id, ActorIds.CULTIST_IDS[0])
...
func test_offer_cigarette_commits_and_needs_a_conscious_patron() -> void:
	...
	assert_gt(session.friendship_value(ScenarioActors.opening_patron(), ActorIds.CULTIST_IDS[0]), before)
	assert_true(
		_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.group_member(&"arrival_group_trio_01", 2, 3)), &"offer_cigarette").is_empty(),
...
func test_knock_out_commits_and_is_disabled_while_another_windup_runs() -> void:
	...
	var issued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"knock_out", _patron_target(ScenarioActors.opening_patron()), false,
		{"is_adjacent": true}
	)
	var started: Dictionary = commands.notify_reached(ActorIds.CULTIST_IDS[0], int(issued["action_id"]))
	assert_false(bool(started["committed"]), "The two-second wind-up starts before impact.")
	assert_eq(session.snapshot()["windup"]["victim_id"], ScenarioActors.opening_patron())
	assert_eq(_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["stage"], &"executing")

	var blocked := _option(commands, ActorIds.CULTIST_IDS[1], _patron_target(ScenarioActors.opening_companion()), &"knock_out")
	...
	commands.notify_reached(ActorIds.CULTIST_IDS[0], int(second["action_id"]))
	session.advance(2.1)
	commands.refresh(ActorIds.CULTIST_IDS[0])
	assert_eq(session.snapshot()["debug_patron_views"][ScenarioActors.opening_patron()]["lifecycle"], &"unconscious")
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty())
...
func test_failed_knockout_replaces_the_queue_with_a_locked_tile_until_stirred() -> void:
	...
	for seed in range(1, 101):
		...
		commands = pair[1]
		_connect_command_refresh(session, commands)
		session.debug_set_patron_drink_state(ScenarioActors.opening_patron(), 0, 5)
		var attempt: Dictionary = commands.issue(
			ActorIds.CULTIST_IDS[0], &"knock_out", _patron_target(ScenarioActors.opening_patron()), false,
			{"is_adjacent": true}
		)
		commands.notify_reached(ActorIds.CULTIST_IDS[0], int(attempt["action_id"]))
		session.advance(2.1)
		commands.refresh()
		if session.cultist_is_incapacitated(ActorIds.CULTIST_IDS[0]):
			break
	assert_true(session.cultist_is_incapacitated(ActorIds.CULTIST_IDS[0]))
	var locked: Dictionary = _queue(commands, ActorIds.CULTIST_IDS[0])["active"]
	...
	var rejected: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"move", _floor_target(1.0, 1.0), false
	)
	assert_false(bool(rejected["accepted"]))
	assert_eq(rejected["reason"], &"cultist_incapacitated")

	var stir: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[1], &"stir", _cultist_target(ActorIds.CULTIST_IDS[0]), false,
		{"is_adjacent": true}
	)
	assert_true(bool(stir["accepted"]))
	commands.notify_reached(ActorIds.CULTIST_IDS[1], int(stir["action_id"]))
	var duplicate: Dictionary = _option(commands, ActorIds.CULTIST_IDS[2], _cultist_target(ActorIds.CULTIST_IDS[0]), &"stir")
	assert_false(bool(duplicate["available"]))
	assert_eq(duplicate["reason"], &"already_helping")
	session.advance(3.1)
	commands.refresh()
	assert_false(session.cultist_is_incapacitated(ActorIds.CULTIST_IDS[0]))
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty())
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[1])["active"].is_empty())
...
func test_pick_up_body_commits_only_for_an_unconscious_patron() -> void:
	...
	assert_true(
		_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.opening_patron()), &"pick_up_body").is_empty(),
	...
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"pick_up_body", _patron_target(ScenarioActors.opening_patron()))["committed"]))
	assert_true(session.snapshot()["drags"].has(ScenarioActors.opening_patron()))
	assert_eq(_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["command"], &"pick_up_body")
	session.advance(1.1)
	commands.refresh(ActorIds.CULTIST_IDS[0])
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty(),
...
func test_drop_body_appears_only_while_a_body_is_carried() -> void:
	...
	assert_true(_option(commands, ActorIds.CULTIST_IDS[0], _floor_target(-6.0, 4.0), &"drop_body").is_empty(),
		"Drop Body Here stays out of the floor menu when nothing is carried.")

	session.debug_set_patron_drink_state(ScenarioActors.opening_patron(), 3, 1, 0, 3)
	session.debug_force_finish_drink(ScenarioActors.opening_patron())
	session.advance(0.2)
	_run(commands, ActorIds.CULTIST_IDS[0], &"pick_up_body", _patron_target(ScenarioActors.opening_patron()))
	session.advance(1.1)
	commands.refresh(ActorIds.CULTIST_IDS[0])

	assert_false(_option(commands, ActorIds.CULTIST_IDS[0], _floor_target(-6.0, 4.0), &"drop_body").is_empty())
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"drop_body", _floor_target(-6.0, 4.0))["committed"]))
	assert_false(session.snapshot()["drags"].has(ScenarioActors.opening_patron()))
...
func test_intercept_appears_only_for_an_escaping_patron() -> void:
	...
	assert_true(_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.friendship_candidate()), &"intercept").is_empty(),
		"A calm Patron offers no Intercept.")

	session.report_patron_stimulus(ScenarioActors.friendship_candidate(), &"drink_dosed_seen")
	session.advance(2.2)
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"intercept", _patron_target(ScenarioActors.friendship_candidate()))["committed"]))
	assert_eq(session.snapshot()["active_intercept"]["patron_id"], ScenarioActors.friendship_candidate())
	assert_eq(_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["command"], &"intercept")
	session.advance(5.1)
	commands.refresh(ActorIds.CULTIST_IDS[0])
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty())
...
func test_lead_to_tunnel_appears_only_at_trusted_friendship() -> void:
	...
	assert_true(
		_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.friendship_candidate()), &"lead_to_tunnel").is_empty(),
	...
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"lead_to_tunnel", _patron_target(ScenarioActors.friendship_candidate()))["committed"]))
	assert_true(session.snapshot()["follows"].has(ScenarioActors.friendship_candidate()))
	assert_eq(_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["command"], &"lead_to_tunnel")
...
func test_rescue_persuasion_targets_the_current_helper_only() -> void:
	...
	assert_true(
		_option(commands, ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.opening_companion()), &"rescue_persuasion").is_empty(),
		"The collapsed Patron is not the Rescue Persuasion target."
	)
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"rescue_persuasion", _patron_target(ScenarioActors.opening_patron()))["committed"]))
	assert_eq(session.snapshot()["collapses"][ScenarioActors.opening_companion()]["phase"], &"persuading")
	assert_eq(_queue(commands, ActorIds.CULTIST_IDS[0])["active"]["command"], &"rescue_persuasion")
	session.advance(6.1)
	commands.refresh(ActorIds.CULTIST_IDS[0])
	assert_true(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty())
...
func test_bar_commands_make_each_drink_type() -> void:
	...
	var options: Array[Dictionary] = commands.resolve_options(
		ActorIds.CULTIST_IDS[0], _object_target(&"bar_work_position")
	)
	...
	var issued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[1], &"make_liquor", _object_target(&"bar_work_position"), false
	)
	commands.notify_reached(ActorIds.CULTIST_IDS[1], int(issued["action_id"]))
	commands.advance(5.0)
...
func test_make_drink_uses_its_five_second_action_duration() -> void:
	...
	var issued: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[0], &"make_beer", _object_target(&"bar_work_position"), false
	)
	var reached: Dictionary = commands.notify_reached(ActorIds.CULTIST_IDS[0], int(issued["action_id"]))
	...
	assert_true(session.snapshot()["prepared_drinks"]["drinks"].is_empty())
	assert_false(_queue(commands, ActorIds.CULTIST_IDS[0])["active"].is_empty())
...
func test_activate_trapdoor_commits_and_reports_its_cooldown() -> void:
	...
	assert_true(bool(_run(commands, ActorIds.CULTIST_IDS[0], &"activate_trapdoor", _object_target(&"trapdoor_control"))["committed"]))
	assert_eq(session.snapshot()["trapdoor"]["state"], &"open")

	var blocked := _option(commands, ActorIds.CULTIST_IDS[1], _object_target(&"trapdoor_control"), &"activate_trapdoor")
...
func test_the_tunnel_intake_offers_no_capture_command_of_its_own() -> void:
	...
	var listed: Array = commands.resolve_options(
		ActorIds.CULTIST_IDS[0], _object_target(&"tunnel_intake")
...
func test_move_stays_the_plain_floor_command() -> void:
	...
	var listed: Array = commands.resolve_options(ActorIds.CULTIST_IDS[0], _floor_target(-6.0, 4.0)).map(
		func(option: Dictionary) -> StringName: return option["command"]
	)
	assert_eq(listed, [&"move"], "Empty floor offers Move alone.")
	var reached := _run(commands, ActorIds.CULTIST_IDS[0], &"move", _floor_target(-6.0, 4.0))
...
func test_normal_snapshots_and_menu_labels_hide_simulation_values() -> void:
	...
	for option: Dictionary in commands.resolve_options(ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.opening_patron())):
...
func test_ten_restarts_leave_no_command_or_reservation_state() -> void:
	...
	for _restart in range(10):
		...
		commands.issue(ActorIds.CULTIST_IDS[0], &"talk", _patron_target(ScenarioActors.opening_patron()), false)
		commands.issue(ActorIds.CULTIST_IDS[1], &"prepare_drink", _object_target(&"bar_work_position"), false)
		commands.reset(session)
...
func test_a_captured_patron_drops_the_command_queued_against_them() -> void:
	...
	var move: Dictionary = commands.issue(
		ActorIds.CULTIST_IDS[1], &"move", _floor_target(-6.0, 4.0), false
	)
	assert_true(bool(commands.issue(ActorIds.CULTIST_IDS[1], &"talk", _patron_target(ScenarioActors.friendship_candidate()), true)["accepted"]))
	_run(commands, ActorIds.CULTIST_IDS[0], &"lead_to_tunnel", _patron_target(ScenarioActors.friendship_candidate()))

	session.advance(15.0)  # the follow reaches the Tunnel Intake and Captures
	assert_eq(int(session.snapshot()["captures"]), 1)
	commands.refresh()
	commands.notify_reached(ActorIds.CULTIST_IDS[1], int(move["action_id"]))

	var queue := _queue(commands, ActorIds.CULTIST_IDS[1])
...
func test_the_night_ending_closes_every_command() -> void:
	...
	assert_true(commands.resolve_options(ActorIds.CULTIST_IDS[0], _patron_target(ScenarioActors.opening_patron())).is_empty(),
