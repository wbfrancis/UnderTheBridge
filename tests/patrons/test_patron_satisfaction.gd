...
func test_each_cultist_can_award_the_first_talk_bonus_once() -> void:
	var satisfaction = SATISFACTION_SCRIPT.new()
	assert_true(satisfaction.complete_talk(ActorIds.CULTIST_IDS[0]))
	assert_false(satisfaction.complete_talk(ActorIds.CULTIST_IDS[0]))
	assert_true(satisfaction.complete_talk(ActorIds.CULTIST_IDS[1]))
