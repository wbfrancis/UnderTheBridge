...
func test_group_gets_exclusive_seats_and_completes_service() -> void:
	...
	assert_eq(state["normal_views"][4]["order_state"], &"served")
	assert_eq(state["normal_views"][5]["order_state"], &"served")
...
func test_seeded_bathroom_checks_are_five_seconds_apart_and_use_empties_bladder() -> void:
	...
	var state: Dictionary = session.snapshot()
	var rolls: Array = state["debug_views"][4]["recent_bathroom_rolls"]
	...
	assert_eq(state["debug_views"][4]["bladder"], 0.0)
	assert_eq(state["bathroom_owner"], ActorIds.NO_ACTOR)
	assert_true(_has_event(state["events"], &"bathroom_visit_completed", 4))
...
func test_intoxication_rises_then_decays_after_four_drink_free_minutes() -> void:
	...
	_serve_opening_orders(session)
	session.advance(43.9)
	assert_eq(session.debug_patron_view(4)["intoxication_level"], 1)
	session.advance(225.0)
	assert_eq(session.debug_patron_view(4)["intoxication_level"], 1)
	session.advance(2.0)
	assert_eq(session.debug_patron_view(4)["intoxication_level"], 0)
...
func test_normal_view_excludes_hidden_values_and_debug_view_exposes_them() -> void:
	...
	_serve_opening_orders(session)
	session.advance(43.9)
	var normal: Dictionary = session.normal_patron_view(4)
	var debug: Dictionary = session.debug_patron_view(4)
...
func test_debug_behavior_fields_are_one_machine_projection() -> void:
	...
	for _transition in range(3):
		var view: Dictionary = session.debug_patron_view(4)
		...
		session.patron_destination_reached(4)
		session.advance(5.1)
	session.restart(707)
	var restarted: Dictionary = session.debug_patron_view(4)
...
func test_completed_talk_identifies_patron_profile_for_the_whole_crew() -> void:
	...
	session.start(707)
	session.advance(1.1)
	var unknown: Dictionary = session.normal_patron_view(4, 1)
	...
	assert_true(session.begin_conversation(1, 4))
...
func test_group_makes_normal_departure_and_releases_seats() -> void:
	...
	var state: Dictionary = session.snapshot()
	assert_eq(state["normal_views"][4]["visible_activity"], "Normal Departure")
	assert_eq(state["normal_views"][5]["visible_activity"], "Normal Departure")
...
func test_physical_bathroom_phases_wait_for_navigation_thresholds() -> void:
	...
	assert_eq(session.debug_patron_view(4)["activity"], &"entering_bathroom")
	assert_true(session.patron_destination_reached(4))
	session.advance(0.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"mirror_check")
	# The walk to the toilet also blocks until navigation reports arrival.
	session.advance(5.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"moving_to_toilet")
	session.advance(30.0)
	assert_eq(session.debug_patron_view(4)["activity"], &"moving_to_toilet")
	assert_true(session.patron_destination_reached(4))
	session.advance(0.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"seated_bathroom_use")
	# The exit travel completes only on its own arrival, at the end of the visit.
	_walk_seated_to_handwashing(session, 4)
	session.advance(5.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"standing_bathroom_exit")
	session.advance(30.0)
	assert_eq(session.debug_patron_view(4)["activity"], &"standing_bathroom_exit")
	assert_true(session.patron_destination_reached(4))
	session.advance(0.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"socializing")
...
func test_physical_closing_departure_has_a_sixty_second_failsafe() -> void:
	...
	assert_eq(session.debug_patron_view(4)["lifecycle"], &"leaving")
	session.advance(59.9)
	assert_eq(session.debug_patron_view(4)["lifecycle"], &"leaving")
	session.advance(0.2)
	assert_eq(session.debug_patron_view(4)["lifecycle"], &"exited")
...
func test_bathroom_line_has_one_waiting_position_and_promotes_its_owner() -> void:
	...
	assert_eq(queued["bathroom_owner"], 4)
	assert_eq(queued["bathroom_line_owner"], 5)
	assert_eq(queued["debug_views"][5]["activity"], &"bathroom_queued")
	...
	assert_eq(promoted["bathroom_owner"], 5)
	assert_eq(promoted["bathroom_line_owner"], ActorIds.NO_ACTOR)
	assert_true(promoted["debug_views"][5]["activity"] in [
...
func test_ideal_intoxication_and_overdrink_limits_are_seeded_and_bounded() -> void:
	...
	replay.start(100)
	assert_eq(replay.debug_patron_view(4)["ideal_intoxication_level"], ideals[0])
...
func test_failed_orders_change_mood_and_suspicion_then_second_failure_leaves() -> void:
	...
	session.start(707)
	session.advance(1.1)
	assert_true(session.debug_set_patron_drink_state(4, 0, 5, 0, 3))
	...
	assert_eq(session.normal_patron_view(4)["order_state"], &"open")
	session.advance(60.1)
	var first: Dictionary = session.debug_patron_view(4)
...
func test_refused_free_drink_is_discarded() -> void:
	...
	for seed in range(100, 200):
		...
		assert_true(candidate.prepare_drink(1))
		candidate.debug_set_patron_drink_state(4, 0, 5, 0, 0)
		var drink_id: StringName = candidate.snapshot()["prepared_drinks"]["drinks"][0]["id"]
		assert_true(candidate.reserve_prepared_drink(drink_id, 1))
		assert_true(candidate.pick_up_prepared_drink(drink_id, 1))
		if candidate.serve_prepared_drink(4, 1, drink_id)["reason"] == &"refused":
	...
	if refused_session == null:
		return
	assert_false(refused_session.carries_prepared_drink(1),
...
func test_overdrink_collapse_uses_excess_limit_and_mood_instead_of_body_suspicion() -> void:
	...
	session.start(707)
	session.advance(1.1)
	assert_true(session.debug_set_patron_drink_state(4, 3, 1, 0, 3))
	var accepted := false
	for _attempt in range(10):
		var offer: Dictionary = session.offer_drink(4, 1)
		if offer["accepted"]:
			accepted = true
			break
		session.advance(60.1)
	assert_true(accepted)
	assert_true(session.debug_force_finish_drink(4))
	var state: Dictionary = session.snapshot()
	var june: Dictionary = state["debug_views"][4]
	var mara: Dictionary = state["debug_views"][5]
	...
	assert_false(session.normal_patron_view(4).has("overdrink_limit"))
...
func test_miserable_companion_does_not_become_collapse_helper() -> void:
	...
	session.start(707)
	session.advance(1.1)
	assert_true(session.debug_change_patron_satisfaction(5, -60.0))
	assert_true(session.debug_set_patron_drink_state(4, 3, 1, 0, 3))
	for _attempt in range(10):
		if session.offer_drink(4, 1)["accepted"]:
			break
		session.advance(60.1)
	assert_true(session.debug_force_finish_drink(4))
	session.advance(2.1)
	assert_eq(session.debug_patron_view(5)["satisfaction_band"], "Miserable")
	assert_eq(session.snapshot()["collapses"][4]["phase"], &"unattended")
...
func test_mirror_check_lasts_five_seconds_then_travels_to_the_toilet() -> void:
	var session = SESSION_SCRIPT.new()
	_drive_into_mirror_check(session, 4)
	session.advance(4.8)
	assert_eq(session.debug_patron_view(4)["activity"], &"mirror_check")
	session.advance(0.3)
	assert_eq(session.debug_patron_view(4)["activity"], &"moving_to_toilet",
...
func test_toilet_arrival_starts_seated_use_and_empties_bladder_at_its_end() -> void:
	...
	session.advance(0.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"seated_bathroom_use")
	var use_seconds: float = session.debug_patron_view(4).get("bathroom_use_seconds", -1.0)
	...
	assert_true(_has_event(session.snapshot()["events"], &"bladder_emptied", 4),
...
func test_sink_arrival_starts_handwashing_for_five_seconds() -> void:
	var session = SESSION_SCRIPT.new()
	_drive_into_handwashing(session, 4)
	assert_eq(session.debug_patron_view(4)["activity"], &"handwashing")
	session.advance(4.8)
	assert_eq(session.debug_patron_view(4)["activity"], &"handwashing")
	session.advance(0.3)
	assert_eq(session.debug_patron_view(4)["activity"], &"standing_bathroom_exit",
...
func test_full_bathroom_visit_releases_the_room_and_promotes_the_line_once() -> void:
	...
	assert_true(session.debug_force_bathroom(4))
	assert_true(session.debug_force_bathroom(5))
	assert_eq(session.snapshot()["bathroom_line_owner"], 5)
	_walk_entering_to_mirror(session, 4)
	_walk_mirror_to_seated(session, 4)
	_walk_seated_to_handwashing(session, 4)
	_walk_handwashing_to_socializing(session, 4)
	var state: Dictionary = session.snapshot()
	assert_eq(_count_events(state["events"], &"bathroom_visit_completed"), 1)
	assert_eq(state["bathroom_owner"], 5,
	...
	assert_eq(state["debug_views"][5]["activity"], &"entering_bathroom")
...
func test_every_standing_bathroom_phase_is_trapdoor_vulnerable() -> void:
	for phase: StringName in [
		...
		var session = SESSION_SCRIPT.new()
		_drive_to_bathroom_phase(session, 4, phase)
		assert_eq(session.debug_patron_view(4)["activity"], phase,
			"Setup reached %s" % phase)
		assert_true(session.activate_trapdoor())
		assert_eq(session.debug_patron_view(4)["activity"], &"trapdoor_falling",
			"%s is a standing phase and begins the Trapdoor fall." % phase)
		session.advance(1.1)  # Fall then panels close.
		assert_eq(session.debug_patron_view(4)["lifecycle"], &"captured",
			"%s is captured once the Trapdoor panels close." % phase)


func test_only_seated_bathroom_use_is_protected_from_the_trapdoor() -> void:
	var session = SESSION_SCRIPT.new()
	_drive_to_bathroom_phase(session, 4, &"seated_bathroom_use")
	assert_eq(session.debug_patron_view(4)["activity"], &"seated_bathroom_use")
	assert_true(session.activate_trapdoor())
	assert_ne(session.debug_patron_view(4)["lifecycle"], &"captured",
		"A seated Patron is protected from the Trapdoor.")
	assert_true(_has_event(session.snapshot()["events"], &"trapdoor_seated_evidence", 4))
...
func test_a_stale_arrival_flag_cannot_complete_a_later_travel_station() -> void:
	...
	session.patron_destination_reached(4)
	session.advance(use_seconds + 0.1)
	assert_eq(session.debug_patron_view(4)["activity"], &"moving_to_sink")
	session.advance(30.0)
	assert_eq(session.debug_patron_view(4)["activity"], &"moving_to_sink",
...
func test_restart_clears_bathroom_visit_and_trapdoor_state() -> void:
	...
	assert_eq(state["debug_views"][4]["activity"], &"entering")
	assert_eq(float(state["debug_views"][4]["bathroom_use_seconds"]), 0.0)
...
func test_trapdoor_fall_ratio_is_bounded_and_clamps_at_one() -> void:
	var session = SESSION_SCRIPT.new()
	_drive_to_bathroom_phase(session, 4, &"handwashing")
	assert_true(session.activate_trapdoor())
	session.advance(0.3)
...
func test_a_queued_patron_promotes_only_after_the_panels_close() -> void:
	...
	session.advance(0.61)
	assert_eq(session.snapshot()["trapdoor"]["state"], &"closing")
	assert_eq(session.snapshot()["debug_views"][5]["activity"], &"bathroom_queued")
	# After closure the room frees and the one waiting Patron promotes.
	session.advance(0.5)
	assert_eq(session.snapshot()["bathroom_owner"], 5)
	assert_eq(session.snapshot()["debug_views"][5]["activity"], &"entering_bathroom")
...
func test_an_investigator_cannot_claim_the_bathroom_while_it_is_locked() -> void:
	...
	assert_true(session.apply_suspicion_stimulus(5, &"missing_companion_40"))
	...
	session.advance(0.1)
	assert_eq(session.snapshot()["debug_views"][5]["activity"], &"waiting_investigation",
...
func _sampled_bathroom_use_seconds(_seed: int) -> float:
	var session = SESSION_SCRIPT.new()
	_drive_to_bathroom_phase(session, 4, &"seated_bathroom_use")
	return session.debug_patron_view(4).get("bathroom_use_seconds", -1.0)
...
func _serve_opening_orders(session) -> void:
	session.advance(1.1)
	assert_true(session.serve_patron_order(4))
	assert_true(session.serve_patron_order(5))
