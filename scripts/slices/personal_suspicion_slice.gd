...
var PATRON_IDS: Array[int] = [ScenarioActors.opening_patron(), ScenarioActors.opening_companion(), ScenarioActors.friendship_candidate()]
const SCENARIOS := {
...
func _set_scenario(scenario_id: String) -> void:
	...
	match scenario_id:
		"independent":
			_session.report_patron_stimulus(ScenarioActors.opening_patron(), &"cancelled_order")
			_session.report_patron_stimulus(ScenarioActors.opening_patron(), &"cancelled_order")
		...
		"hard_evidence":
			...
			_session.report_patron_stimulus(ScenarioActors.opening_companion(), &"drink_dosed_seen")
		...
		"max_drunk":
			_session.report_patron_stimulus(ScenarioActors.opening_patron(), &"knockout_witnessed", true)
			_session.advance(30.0)
			_session.report_patron_stimulus(ScenarioActors.opening_patron(), &"trapdoor_capture_witnessed", true)
		...
		"maximum_selection":
			...
			_session.report_patron_stimulus(ScenarioActors.opening_companion(), &"missing_companion_40")
...
func _validation_report() -> Dictionary:
	...
	independent.report_patron_stimulus(ScenarioActors.opening_patron(), &"cancelled_order")
	independent.report_patron_stimulus(ScenarioActors.opening_patron(), &"cancelled_order")
	...
	recovery.report_patron_stimulus(ScenarioActors.opening_patron(), &"cancelled_order")
	recovery.report_patron_stimulus(ScenarioActors.opening_patron(), &"cancelled_order")
	recovery.advance(29.9)
	var recovery_before: float = recovery.snapshot()["debug_patron_views"][ScenarioActors.opening_patron()]["suspicion"]
	recovery.advance(0.1)
	var recovery_first: float = recovery.snapshot()["debug_patron_views"][ScenarioActors.opening_patron()]["suspicion"]
	recovery.advance(10.0)
	var recovery_second: float = recovery.snapshot()["debug_patron_views"][ScenarioActors.opening_patron()]["suspicion"]

	var hard = GAME_SESSION_SCRIPT.new()
	hard.start_night(707)
	hard.advance(190.0)
	hard.report_patron_stimulus(ScenarioActors.opening_companion(), &"drink_dosed_seen")
	hard.advance(120.0)
	var hard_debug: Dictionary = hard.snapshot()["debug_patron_views"][ScenarioActors.opening_companion()]

	var drunk = GAME_SESSION_SCRIPT.new()
	drunk.start_night(707)
	drunk.advance(190.0)
	drunk.report_patron_stimulus(ScenarioActors.opening_patron(), &"knockout_witnessed", true)
	drunk.advance(30.0)
	drunk.report_patron_stimulus(ScenarioActors.opening_patron(), &"trapdoor_capture_witnessed", true)
	var drunk_debug: Dictionary = drunk.snapshot()["debug_patron_views"][ScenarioActors.opening_patron()]

	var selection = GAME_SESSION_SCRIPT.new()
	selection.start_night(707)
	selection.advance(190.0)
	selection.report_patron_stimulus(ScenarioActors.opening_patron(), &"drink_dosed_seen")
	selection.report_patron_stimulus(ScenarioActors.opening_companion(), &"missing_companion_40")
	var selection_debug: Dictionary = selection.snapshot()["debug_patron_views"]
	var checks := {
		"independent_bands": independent_state["normal_patron_views"][ScenarioActors.opening_patron()]["suspicion_band"] == "Uneasy" and independent_state["normal_patron_views"][ScenarioActors.opening_companion()]["suspicion_band"] == "Calm",
		"normal_hides_exact_value": not independent_state["normal_patron_views"][ScenarioActors.opening_patron()].has("suspicion"),
		...
		"maximum_response_selection": selection_debug[ScenarioActors.opening_patron()]["suspicion_maximum_response"] == &"escape" and selection_debug[ScenarioActors.opening_companion()]["suspicion_maximum_response"] == &"investigation",
	}
	return {
