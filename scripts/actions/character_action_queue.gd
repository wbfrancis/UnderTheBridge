class_name CharacterActionQueue
extends RefCounted

## One ordered Action Queue for one character: a single active Action and any
...
# Marks the active Action past its Commitment Point without advancing a second
# clock. Session-owned operations use this after their gameplay effect starts.
func mark_active_committed(action_id: int) -> bool:
	if _active.is_empty() or int(_active["id"]) != action_id:
		return false
...
## Replaces every unfinished Action with one uncancellable state Action. This is
## used when an effect incapacitates a character and invalidates all
## player intent that was queued before the fall.
func force_lock(
		name: StringName,
		target_id: Variant,
	...
	if not _active.is_empty():
		_record_event(_active, &"cancelled", &"cultist_incapacitated")
		removed.append(_active.duplicate(true))
	for action in _pending:
		_record_event(action, &"cancelled", &"cultist_incapacitated")
		removed.append(action.duplicate(true))
	_pending.clear()
	_paused.clear()
	_active = _make_action(
		name, target_id, duration_seconds, 0.0, true, payload
	)
	_active["state"] = &"committed"
	return {"action_id": int(_active["id"]), "removed": removed}


func clear_forced_lock(name: StringName) -> bool:
	if _active.is_empty() or StringName(_active["name"]) != name:
		return false
...
## Pauses the current Action without changing its ID or elapsed time, then runs
## one interrupt Action. When the interrupt ends, the paused Action resumes.
func interrupt_with(action_spec: Dictionary) -> int:
	if not _active.is_empty():
		_paused.push_front(_active)
	_active = _action_from_spec(action_spec, -1, 0, 1)
	return int(_active["id"])


## Debug-only removal after a Commitment Point. Gameplay effects that already
## fired stay applied; this only ends the remaining Action lifecycle.
func force_remove_active(reason: StringName) -> bool:
	if _active.is_empty():
		return false
...
func remove_paused(action_id: int) -> Dictionary:
	for index in range(_paused.size()):
		if int(_paused[index]["id"]) == action_id:
			var action: Dictionary = _paused.pop_at(index)
			_record_event(action, &"cancelled", &"debug_cancelled")
			return action.duplicate(true)
	return {}


## Atomically removes active, pending, and paused work. No intermediate Action
## is activated, so a terminal transition cannot revive interrupted behavior.
func clear_all(reason: StringName = &"debug_cancelled") -> Array[Dictionary]:
	var removed: Array[Dictionary] = []
	if not _active.is_empty():
		removed.append(_active.duplicate(true))
	removed.append_array(_pending.duplicate(true))
	removed.append_array(_paused.duplicate(true))
	for action: Dictionary in removed:
...
## Coordinators that own completion effects use this clock without allowing the
## queue to discard the Action before the effect is applied.
func advance_clock(simulated_seconds: float) -> void:
	if not _active.is_empty() and simulated_seconds > 0.0:
		_active["elapsed_seconds"] += simulated_seconds


func configure_active_timing(duration: float, commitment: float) -> void:
	if _active.is_empty():
		return
	_active["duration_seconds"] = duration
	_active["commitment_seconds"] = commitment


func force_active_ready() -> bool:
	if _active.is_empty():
		return false
	var duration := float(_active["duration_seconds"])
	_active["elapsed_seconds"] = duration if is_finite(duration) else 1.0e9
	return true
...
func _is_active_committed() -> bool:
	return not _active.is_empty() and _active["state"] == &"committed"


func _activate_next() -> void:
	if not _paused.is_empty():
		_active = _paused.pop_front()
		return
	if _pending.is_empty():
		_active = _paused.pop_front() if not _paused.is_empty() else {}
		return
